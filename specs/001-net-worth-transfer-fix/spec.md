...
4. **Given** a transfer involves an account that is excluded from net worth tracking (e.g., an off-budget or closed account), **When** the user views the net worth graph spanning the transfer's dates, **Then** the graph is NOT neutralized: the leg belonging to the included (tracked) account is shown as a genuine net worth change, since money has genuinely moved into or out of the tracked set of accounts.
...
- What happens when a transfer occurs between an account tracked in net worth and one that is not (e.g., a tracking-only or off-budget account)? The transfer is NOT neutralized in this case; only the leg belonging to the tracked account is shown, as a genuine net worth change.
...
- How does the system handle a net worth graph view whose date range starts or ends between the two mismatched transfer-leg dates (i.e., only one leg of the transfer falls within the viewed range)? The visible leg is shown as a real balance change; transfer-neutral treatment only applies when both legs of the transfer fall inside the viewed range (or when viewing all-time). The system does not fabricate an offset from a leg the user is not viewing.
- What happens when a transfer involves a currency conversion, exchange-rate spread, or an explicit transfer fee? Only the matched transferred principal is neutralized; any fee or conversion difference remains visible as a genuine net worth change. The fix must never suppress a real cost.
- What counts as a "transfer" for purposes of this fix? Only the two transactions explicitly linked by Actual Budget's transfer feature (via `transfer_id`). Manually entered, unlinked matching inflow/outflow pairs are not detected or neutralized by this fix.
- What happens if a transfer's link between its two legs is broken, or one leg is missing/deleted, leaving an orphaned transaction? The surviving transaction is treated as a normal, non-neutral transaction affecting net worth — there is no pair to offset, so it counts as a real balance change with no residual neutral treatment.
...
- **FR-007**: A transfer MUST be treated as zero net-worth impact only when both linked accounts are included in net worth tracking. If one of the two accounts is excluded from net worth tracking (e.g., off-budget, closed, or tracking-only), the fix MUST NOT neutralize the transfer; the leg belonging to the included account MUST continue to show as a genuine net worth change.
- **FR-008**: When a viewed date range contains only one leg of a mismatched-date transfer, the system MUST show that visible leg as a real balance change and MUST NOT fabricate an offsetting adjustment for the leg outside the viewed range. Transfer-neutral treatment MUST apply only when both legs fall within the viewed range (or when viewing all-time).
- **FR-009**: For transfers involving a currency conversion, exchange-rate spread, or explicit transfer fee, the fix MUST neutralize only the matched transferred principal, leaving any fee or conversion difference visible as a genuine net worth change.
- **FR-010**: The zero-impact rule MUST apply only to the two transactions explicitly linked by Actual Budget's transfer feature (via `transfer_id`); manually entered, unlinked matching inflow/outflow pairs are out of scope and MUST NOT be treated as transfers by this fix.
- **FR-011**: If a transfer's link is broken or one leg is missing/deleted, the remaining (orphaned) transaction MUST be treated as a normal, non-neutral transaction affecting net worth, with no residual transfer-neutral treatment.
...
- Transfers between two accounts that are both included in net worth tracking are the primary scope of this fix; when one leg's account is excluded from net worth tracking, the transfer is not neutralized and the tracked leg is shown as a genuine net worth change (confirmed by BA).
...
- Only Actual Budget's explicitly-linked transfers (via `transfer_id`) are in scope; manually entered, unlinked matching inflow/outflow pairs are not detected or neutralized (confirmed by BA).
- When a viewed date range contains only one leg of a mismatched-date transfer, no offset is fabricated for the out-of-range leg; the visible leg shows as a real balance change, and neutralization only applies when both legs are within the viewed range or the view is all-time (confirmed by BA).
- For transfers with a currency conversion, exchange-rate spread, or fee, only the matched principal is neutralized; any fee or conversion difference remains a genuine, visible net worth change (confirmed by BA).
- If a transfer's link is broken or one leg is deleted, the surviving leg is treated as a normal, non-neutral transaction with no residual transfer treatment (confirmed by BA).
